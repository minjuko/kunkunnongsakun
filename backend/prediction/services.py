...
def _parse_response(response, service_name):
    ...
    try:
        ...
        raise ServiceUnavailableError(f'{service_name} returned an invalid JSON response.') from exc


def fetch_market_prices(crop_name, region, start_date, end_date, price_code, region_codes):
    service_key = unquote(_required_env('DATA_GO_KR_MARKET_SERVICE_KEY', 'Market price service'))
    ...
    while page_no <= MARKET_MAX_PAGES:
        ...
        try:
            ...
            raise ServiceUnavailableError(
                'Market price service returned malformed pagination data.'
            )
...
def fetch_weather_data(region, region_codes):
    service_key = unquote(_required_env('DATA_GO_KR_WEATHER_SERVICE_KEY', 'Weather service'))
    ...
    rows = [
        {child.tag: child.text for child in item if child.tag in columns}
